...
6. **Operator workflow depth**
   ...
   - Shipped: closed-loop adaptation — past PROs inform mission selection (avoid templates reported "harder" 2+ times, prefer templates with "easier" + "noticed difference"). The loop is visible to the patient: Mira's opening line references past outcomes and signals adaptation when offering a new mission. Outcome-aware Mira flags (struggle + positive signal). Longitudinal outcome trend in clinician digest.
