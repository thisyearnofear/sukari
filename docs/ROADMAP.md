...
6. **Operator workflow depth**
   ...
   - Shipped: closed-loop adaptation — past PROs inform mission selection (avoid templates reported "harder" 2+ times, prefer templates with "easier" + "noticed difference"). The loop is visible to the patient: Mira's opening line references past outcomes AND the patient's free-form reflection, and signals adaptation when offering a new mission. Milestone language marks the first completion and the first noticed-difference. Outcome-aware Mira flags (struggle + positive signal). Longitudinal outcome trend in clinician digest.
